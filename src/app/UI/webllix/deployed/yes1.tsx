          ...
          {loading ? (
            ...
            incomingImages.map((image, index) => (
              <div key={image.id} className="relative">
                <img
                  src={`/api/users/generate?id=${image.id}`}
                  alt={`Generated image ${index + 1}`}
                  className="w-full h-auto cursor-pointer"
                  onClick={() => fetchAndSetSelectedImage(image.id)}
                />
...
///its displaying iamge id and sending to openia generatign code bas ehtisiamge sometime work sometime not  but difretn image id  
  ...
  try {
    ...
    if (action === "create_accounts") {
      ...
      const results = await Promise.all(
        platforms.map(async (platform: string) => {
          // Fetch the auth token for each platform from your database
...
async function fetchTokenFromDatabase(platform: string): Promise<string> {
  // Implement this function to retrieve the stored token for each platform
  // from your database
  return "stored_token_for_" + platform;
}


//now correctly redirecting to the page
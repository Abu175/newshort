...
export async function fetchTokenFromDatabase(db: Db, platform: string): Promise<PlatformTokenData> {
  ...
  if (result) {
    return result as PlatformTokenData;
  } else {
...
export async function createAccountAndAuthorize(platform: string): Promise<{ redirectUrl: string }> {
  ...
  switch (platform) {
    ...
    case "youtube":
      ...
      const redirectUri = `${process.env.APP_URL}/api/users/auth/youtube/callback`;
      return {
        redirectUrl: `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${redirectUri}&scope=https://www.googleapis.com/auth/youtube.upload&response_type=code&access_type=offline&prompt=consent`
      };
    case "reddit":
      return {
...
async function exchangeYouTubeCode(code: string): Promise<PlatformTokenData> {
  const clientId = process.env.YOUTUBE_CLIENT_ID;
...
//its taking corectly but end itegetitng failed
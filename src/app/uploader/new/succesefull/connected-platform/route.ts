...
async function uploadToPlatform(videoBuffer: Buffer, platform: string, token: PlatformAuth.PlatformTokenData) {
  ...
  switch (platform) {
    ...
    case "youtube":
      ...
      const fileMetadata = {
        snippet: {
          title: 'Video uploaded via My App new way',
  ...
  if (["instagram", "facebook", "twitter", "linkedin", "pinterest", "reddit"].includes(platform)) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    let action, platforms, code, platform, video;
    ...
    if (contentType && contentType.includes("multipart/form-data")) {
      ...
      ({ action, platforms, code, platform } = body);
    }
